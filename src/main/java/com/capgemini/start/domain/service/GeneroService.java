...
public class GeneroService extends AbstractService<Genero, Long> {
	...
	public Genero findById(Long id) {
		return repository.findById(id).orElseThrow(() -> new ObjectNotFoundException("Genero não localizado"));
	}
	
	@Override
	public Genero insert(Genero genero) {
		if(this.repository.existsByDescricaoIgnoreCase(genero.getDescricao())) {
			throw new ObjectAlreadyExistsException("Essa descrição já existe em outro Genero");
		}
